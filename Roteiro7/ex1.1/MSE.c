...
void subMatrizes(Matriz* A, Matriz* B, Matriz* C) {
    if(dimIguais(A, B)) {
        C = criaMatriz(A->lin, A->col);
        for (int i = 0; i < C->lin; i++) {
            for (int j = 0; j < C->col; j++) {
...
void multMatrizes (Matriz* A, Matriz* B, Matriz* C) {
    int mult = 0;
    if(multPossivel(A, B)) {
        C = criaMatriz(A->lin, B->col);
         for (int i = 0; i < C->lin; i++) {
            for (int j = 0; j < C->col; j++) {
                for (int k = 0; k < A->col; k++) {
                    for (int l = 0; l < B->lin; l++) {
                    mult += A[i][k]*B[j][l];
                    }
                }
                C[i][j] = mult;
...
void mensagemErro() {
    printf (" - Nao eh possivel realizar a operacao devido as dimensoes das matrizes!\n");
}

int dimIguais (Matriz* A, Matriz* B) {
    if (A->lin == B->lin && A->col == B->col) return 1;
    return 0;
}

int multPossivel (Matriz* A, Matriz* B) {
    if (A->col == B->lin) return 1;
    return 0;
}
